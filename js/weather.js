...
(function () {
  ...
  el.innerHTML = '<div class="wx-meta">Loading weather…</div>';
  ...
  url.search = new URLSearchParams({
    ...
    longitude: lon,
    timezone: tz,
    // Legacy "current_weather" is fine and very light:
    current_weather: 'true',
    // IMPORTANT: use weather_code (with underscore)
  ...
  const render = (data) => {
    ...
    const hCode = data.hourly.weather_code || []; // <-- underscore

    // find the first hourly slot >= now
    const nowTs = Date.now();
    let idx = hTime.findIndex(t => new Date(t).getTime() >= nowTs);
    if (idx < 0) idx = 0;
    ...
    const dCode = data.daily.weather_code || []; // <-- underscore
    ...
    dDay.forEach((iso, i) => {
      ...
      grid.appendChild($(`
        <div class="wx-day" title="${dsc}">
          ...
          <div class="precip">${(dPre[i] ?? 0) > 0 ? dPre[i].toFixed(1) + ' mm' : ''}</div>
        </div>
      `));
    });
    root.append(days);
