...
(function() {
  ...
  function googleTranslateElementInit() {
    new google.translate.TranslateElement({
      pageLanguage: 'en',
      includedLanguages: 'en,no,de,fr,es,it,pl,ru,zh-CN,ja',
      autoDisplay: false
  ...
  // 2) Popover logic + wiring to Google’s hidden select
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // When a language is clicked, set the hidden select value and dispatch change
    menu.querySelectorAll('.translate-item').forEach(it => {
      it.addEventListener('click', () => {
        const lang = it.dataset.lang;
        const sel = document.querySelector('.goog-te-combo'); // Google injects this
        if (sel) {
          sel.value = lang;
          sel.dispatchEvent(new Event('change'));
          closeMenu();
        } else {
          alert('Translator not ready yet—please try again in a second.');
        }
      });
    });
  });
...
/* Top button function */
// Get the button:
let top_button = document.getElementById("top-btn");
